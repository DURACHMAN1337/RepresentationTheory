...
public class SpnC {
    ...
    public static void main(String[] args) {
        SpnC spnC = new SpnC();
        ArrayList<int[][]> listF = spnC.generateMatricesF(4);
        ArrayList<int[][]> listH = spnC.generateMatricesH(4);
        ArrayList<GeomVector> vectorsA = generateVectorsA(listH, listF);
    ...
    public ArrayList<int[][]> generateMatricesF(int dim) {
        ArrayList<int[][]> result = new ArrayList<>();
        if (dim % 2 == 0) {
            int[][] matrix_fi;
            int[][] matrix_fij_1;
...
//                    matrix_fij_2 = new int[dim][dim];
                    if (i != j) {
                        matrix_fij_1[i][j] = 1;
                        matrix_fij_1[(dim - 1) - j][(dim - 1) - i] = -1;

//                        matrix_fij_2[i][j] = -1;
//                        matrix_fij_2[(dim - 1) - j][(dim - 1) - i] = 1;

                        result.add(matrix_fij_1);
//                        result.add(matrix_fij_2);
    ...
    public static ArrayList<GeomVector> generateVectorsA(ArrayList<int[][]> h, ArrayList<int[][]> f) {
        ...
        for (int i = 0; i < h.size(); i++) {
            for (int[][] matrix : basis) {
                int[][] newMatrix = matricesMinus(multiplyMatrices(h.get(i), matrix), multiplyMatrices(matrix, h.get(i)));
                temp.add(newMatrix);
                if (Arrays.deepEquals(newMatrix, nullMatrix)) {
                    ...
                    if (Arrays.deepEquals(newMatrix, matrix)) {
                        vectorsA.add(1);
                    } else if (Arrays.deepEquals(newMatrix, multiplyMatrixByNumber(matrix, -1))) {
                        vectorsA.add(-1);
                    } else {
                        vectorsA.add(2);
                    }
    ...
    public static int[][] multiplyMatrixByNumber(int[][] matrix, int num) {
